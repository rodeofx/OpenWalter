...
//
#ifndef WALTERHYDRA_GENERATED_TEXTURE_H
#define WALTERHYDRA_GENERATED_TEXTURE_H
...
/// \class WalterHdStreamTexture
///
///
class WalterHdStreamTexture : public UsdShadeShader
{
public:
    ...
    /// Construct a WalterHdStreamTexture on UsdPrim \p prim .
    /// Equivalent to WalterHdStreamTexture::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit WalterHdStreamTexture(const UsdPrim& prim=UsdPrim())
        : UsdShadeShader(prim)
    {
    }

    /// Construct a WalterHdStreamTexture on the prim held by \p schemaObj .
    /// Should be preferred over WalterHdStreamTexture(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit WalterHdStreamTexture(const UsdSchemaBase& schemaObj)
    ...
    /// Return a WalterHdStreamTexture holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// WalterHdStreamTexture(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDHYDRA_API
    static WalterHdStreamTexture
