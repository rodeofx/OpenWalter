...
//
#ifndef WALTERHYDRA_GENERATED_UVTEXTURE_H
#define WALTERHYDRA_GENERATED_UVTEXTURE_H
...
/// as the value.
///
class WalterHdStreamUvTexture : public WalterHdStreamTexture
{
public:
    ...
    /// Construct a WalterHdStreamUvTexture on UsdPrim \p prim .
    /// Equivalent to WalterHdStreamUvTexture::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit WalterHdStreamUvTexture(const UsdPrim& prim=UsdPrim())
        : WalterHdStreamTexture(prim)
    {
    }

    /// Construct a WalterHdStreamUvTexture on the prim held by \p schemaObj .
    /// Should be preferred over WalterHdStreamUvTexture(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit WalterHdStreamUvTexture(const UsdSchemaBase& schemaObj)
        : WalterHdStreamTexture(schemaObj)
    {
    }

    /// Destructor.
    USDHYDRA_API
    virtual ~WalterHdStreamUvTexture();
    ...
    /// Return a WalterHdStreamUvTexture holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// WalterHdStreamUvTexture(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    USDHYDRA_API
    static WalterHdStreamUvTexture
